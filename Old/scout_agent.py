    ...
    def extract_prices(self, chart_data: Dict) -> Optional[List[float]]:
        ...
        try:
            ...
            if len(prices) < 10:
                ...
                # Check if this might be a market hours issue
                ...
                if ticker_symbol.endswith('.NS') or ticker_symbol.endswith('.BO'):
                    ...
                    if not is_weekday:
                        logger.error("❌ Indian stock market is closed (Weekend)")
                        return None
                    elif not market_open:
                        logger.error(f"❌ Indian stock market is closed (Market hours: 9:15 AM - 3:30 PM IST, Current: {current_time.strftime('%I:%M %p')})")
                        return None
